...
myip=$(ifconfig | grep -Eo 'inet (dirección:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\. ){3}[0-9]*' | grep -v '127.0.0' | cabeza -n1)
...
mensaje() {
  ...
  "-barra2" | "-barra") cor="${VERMELHO}—————————————————————————————————————— ——————————————" && echo -e "${SEMCOR}${cor}${SEMCOR}" ;;
  esac
}
fun_bar() {
  comando="$1"
